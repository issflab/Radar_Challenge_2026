...
class deep_learning:
    ...
    def __init__(self, model_name, device='cpu', model_path='wavlm_large.pt'):
    # def __init__(self, model_dir="wavlm", device='cpu'):

        self.SAMPLE_RATE = 16000
        self.MAX_SECONDS = 60

        # Use model from custom upstream path
        # self.model = S3PRLUpstream(model_dir).to(device)
        # self.featurizer = Featurizer(self.model).to(device)

        self.model = S3PRLUpstream(model_name, path_or_url=model_path).to(device)
