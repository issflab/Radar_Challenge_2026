...
class SSLModel(nn.Module):
    def __init__(self, device, args, ssl_feature):
        super(SSLModel, self).__init__()
        
        self.device = device

        out_dim_dict = {'wavlm_large': 1024, 'mae_ast_frame':768, 'npc_960hr':512}
        self.out_dim = out_dim_dict[ssl_feature]

        model_path = ssl_feature + '.pt'
        print(model_path)

        self.model = deep_learning(model_name=ssl_feature, model_path=model_path, device=device)

        # if ssl_feature == 'wavlm_large':
        #     self.model = deep_learning(model_name=ssl_feature, device=device, model_path='wavlm_large.pt') # or 'wav2vec2_base', as you prefer
        #     # self.model = deep_learning(model_dir="wavlm", device=device)
        #     # wavlm_large -> 1024, mae_ast_frame -> 768
        #     self.out_dim = 1024  # Default for hubert_base (for xlsr2_300m -> 1024, but hubert -> 768)
        
        # if ssl_feature == 'mae_ast_frame':
        #     self.model = deep_learning(model_name=ssl_feature, device=device, model_path='mae_ast_frame.pt') # or 'wav2vec2_base', as you prefer
        #     self.out_dim = 768  # Default for hubert_base (for xlsr2_300m -> 1024, but hubert -> 768)
...
class Model(nn.Module):
    def __init__(self, args, device, ssl_feature):
        super().__init__()
        self.device = device
        self.args = args
        ...
        self.ssl_feature = ssl_feature

        ####
        # create network wav2vec 2.0
        ####
        self.ssl_model = SSLModel(self.device, args, self.ssl_feature)
